...
import { transaccionConReintento } from '../helpers/transacccion';
...
export default class Actividad extends Model {
  ...
  async sacarTurnoPara({ usuario, ...atributosTurno }) {
    return transaccionConReintento(this.sequelize, async () => {
      if (await usuario.tieneMasDeDosTurnos(this.fechaHoraInicio)) {
        throw new TieneMasDeDosTurnosError();
      }
      ...
      if ((await this.lugaresDisponibles()) <= 0) {
        throw new ActividadSinCuposError();
      }
      return Turno.create({
...
export class ActividadSinCuposError extends DominioError {
  constructor() {
...
export class TieneMasDeDosTurnosError extends DominioError {
  constructor() {
    super('Ya tenés dos turnos para este día');
  }
}
