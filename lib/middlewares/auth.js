...
import { buildMessage, sendEmail } from '../helpers/mail_sender';
...
import { subjectRegister } from '../config/mail_sender';
  ...
  new LocalStrategy(
    ...
    async (req, dni, contrasenia, done) => {
      try {
        ...
        await sendEmail(buildMessage(usuario.email, subjectRegister));
