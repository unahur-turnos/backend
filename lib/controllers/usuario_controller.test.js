...
describe('Usuario controller', () => {
  ...
  beforeAll(async () => {
    ...
    MailSender.sendMail.mockResolvedValue();
  ...
  describe('/registro', () => {
    ...
    it('Registro con número de telefono grande', async () => {
      const response = await request.post('/api/usuarios/registro').send({
        nombre: 'Im a',
        apellido: 'Test',
        contrasenia: 'admin123',
    ...
    it('Verificar envío de mail', async () => {
      const response = await request.post('/api/usuarios/registro').send({
        nombre: 'Usuario',
        apellido: 'Prueba',
        contrasenia: 'Admin123',
      ...
      expect(response.statusCode).toBe(201);
      expect(MailSender.sendMail.mock.calls.length).toBeLessThanOrEqual(3);

      // mock.calls[2] devuelve un array con 3 elementos: [email, subject, html]
      expect(MailSender.sendMail.mock.calls[2]).toEqual(
        expect.arrayContaining([
          'usuario@pruebaturnos.com',
