...
import { compose, concat, drop, map, prop, propOr } from 'ramda';
...
export const recuperarContrasenia = async (req, res) => {
  ...
  const templateRecuperarContrasenia = await HTMLUtils.getHTMLFile(
    'lib/html/templateRecuperarContrasenia.html',
    {
      ...
      linkTemporal: concat(host, `/usuario/${usuario.dni}/recuperar/${token}`),
    }
  );
  ...
  // TODO: propuesta de interfaz de sendMail
  //
  // await sendMail({
...
export const actualizarContrasenia = async (req, res) => {
  try {
    // TODO: hacer esto de una mejor forma, debe haber algún paquete que lo parsea. Ver https://github.com/reecefenwick/express-auth-parser
    // TODO: guardar token temporal en usuario
    const tokenTemporal = drop(7, req.headers.authorization);
