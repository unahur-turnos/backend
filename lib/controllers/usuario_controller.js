import Usuario, { ContraseniaIncorrectaError } from '../models/usuario';
import { asuntoRecuperarContrasenia, host } from '../config/mail_sender';
import { compose, concat, map, pick, prop, propOr } from 'ramda';
...
export const actualizarUsuario = async (req, res) => {
  ...
  if (!usuario) {
    return res
      .status(400)
      .json({ error: `No existe un usuario con el id ${id}` });
  }

  if (!usuario.validarContrasenia(datosUsuario.contrasenia)) {
    throw new ContraseniaIncorrectaError();
  }

  await usuario.update(pick(['email', 'telefono'], datosUsuario));
  const token = generarJWT(id, datosUsuario.dni);
