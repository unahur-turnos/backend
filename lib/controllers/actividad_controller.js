...
export const index = async (req, res) => {
  ...
  const data = await Actividad.findAll({
    ...
    where: getCondicion(fecha),
  });

  res.send({ data });
};

const getCondicion = (fecha) => {
  if (fecha !== undefined) {
    const inicio = DateTime.fromISO(fecha).startOf('day').toISO();
    const fin = DateTime.fromISO(fecha).endOf('day').toISO();
    return { fechaHoraInicio: { [Op.between]: [inicio, fin] } };
  }
  return {};
};
